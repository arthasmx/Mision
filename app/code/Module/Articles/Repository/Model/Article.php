...
class Module_Articles_Repository_Model_Article extends Core_Model_Repository_Model{
  ...
  function read_full_article($article_seo_OR_id = 'not_given!', $enabled_only=null, $throw_exceptions = false){
    $article = $this->core->_db->select()
                    ->from(array('va' => 'admin_view_articles' ) )
                    ->where( 'va.language = ?', App::locale()->getLang() )
  ...
  function jqGrid_admin_list( $params=null ){
    ...
    $select = $this->core->_db
                         ->select()
                         ->from( array('va' => 'vista_articles'), array('article_id','title','seo','created','publicated','event_date','stop_publication','author','type_name','status','language','promote','mobile') )
                         ->where('va.type != ?', App::xlat('section_page') );
  ...
  /*
   * @todo: Modificar esta consulta para que obtenga los datos desde vista_categories
   * problema: el idioma del seo de las categorias
   */
  function jqGrid_record_types(){
    $select  = $this->core->_db->select()->from( array('va' => 'vista_articles'), array('type'=>'DISTINCT(type)','type_name') )->where('va.type != ?', App::xlat('section_page') );
